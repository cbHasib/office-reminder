/* Web Audio synthesis of the five built-in sounds.
   Mirror of apps/web/components/soundPreview.ts. */
...
const RECIPES: Record<SoundName, (ctx: AudioContext) => void> = {
  ...
  bell:  (ctx) => tone(ctx, 880, 0, 0.25, 0.9, "triangle"),
  ding:  (ctx) => tone(ctx, 1320, 0, 0.10, 0.25, "sine"),
  soft:  (ctx) => {
    tone(ctx, 392, 0, 0.30, 0.8, "sine");
...
function tone(
  ctx: AudioContext, freq: number, startOffset: number, attack: number,
  total: number, type: OscillatorType,
) {
